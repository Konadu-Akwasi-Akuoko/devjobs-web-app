...
describe("Footer", () => {
  it("should render the position, company and apply button of the clicked post data", () => {
    const mockData = {
      id: 1,
      company: "Scoot",
    ...
    render(<Footer clickedPostData={mockData} />);

    expect(screen.getByText(mockData.position)).toBeInTheDocument();
    expect(
      screen.getByText(mockData.company + " Digital Inc.")
    ).toBeInTheDocument();
